...
## 3  Version 3 : Multi-critères et historique

### 3.1  Lancement et utilisation

Le lancement reste identique aux versions précédentes. Les nouvelles fonctionnalités sont accessibles directement via les classes du modèle. Le fichier d'historique est un fichier binaire `.ser` généré automatiquement par `HistoriqueManager` dans le répertoire.

Exemple d'utilisation depuis le code :

```java
// Créer un voyageur avec préférences multi-critères
...
Voyageur voyageur = new Voyageur("Alice", prefs);

// Obtenir les trajets triés par coût
List<Trajet> tries = plateforme.getTrajetsTriesParPreferences(voyageur);

// Enregistrer un voyage dans l'historique
Voyage voyage = new Voyage(tries);
voyageur.ajouterVoyage(voyage);

// Persister l'historique
HistoriqueManager manager = new HistoriqueManager("historique.ser");
manager.ajouterEtSauvegarder(voyage);
```

---

### 3.2  Diagramme UML et mécanismes objets

**Tableau 4 : Nouveautés de la Version 3**

| Élément ajouté / modifié | Justification (sujet) |
|---|---|
| `Voyageur.preferences` — `Map<TypeCout, Double>` | Exprimer les préférences entre les critères |
| `Voyageur.historique` — `List<Voyage>` | Enregistrer les voyages effectués par l'utilisateur |
| `Voyageur.calculerCoutComposite(Cout)` | Adapter le modèle de calcul pour une combinaison de critères |
| `Voyageur` implements `Serializable` | Permettre la sérialisation binaire de l'utilisateur |
| `Voyage` | Représenter un voyage effectué avec ses coûts totaux |
| `HistoriqueManager` | Gérer la persistance de l'historique via sérialisation binaire |
| `Plateforme.getTrajetsTriesParPreferences(Voyageur)` | Trier les trajets selon les préférences de l'utilisateur |

*[Diagramme UML V3 — à insérer ici (fichier diagramme_uml_v3.puml)]*

#### Mécanismes objets mis en œuvre

**Surcharge de constructeur** : `Voyageur` dispose maintenant de deux constructeurs : un pour la compatibilité V1/V2 (un seul `TypeCout`) et un pour la V3 (une `Map<TypeCout, Double>`). Ce mécanisme permet de ne pas casser le code existant tout en ajoutant la nouvelle fonctionnalité.

**Sérialisation** : Les classes `Voyage`, `Voyageur`, `Trajet`, `Ville` et `Cout` implémentent désormais `Serializable`. Ce mécanisme Java permet de convertir les objets en flux binaire pour les écrire dans un fichier, puis de les reconstruire à l'identique à la lecture. C'est la solution la plus simple pour persister l'historique sans base de données.

**Normalisation des préférences** : La méthode privée `normaliserPreferences()` s'assure que la somme des poids est toujours égale à 1, quelle que soit l'entrée de l'utilisateur. Cela garantit que `calculerCoutComposite` retourne toujours un coût cohérent et comparable entre voyageurs.

**Délégation** : `HistoriqueManager` est une classe dédiée à la persistance : elle ne connaît pas `Plateforme` ni `Voyageur`, seulement `Voyage`. Ce découplage respecte le principe de responsabilité unique : chaque classe fait une seule chose.

---

### 3.3  Analyse technique des fonctionnalités

**Préférences multi-critères** : Le coût composite d'un trajet est calculé par `calculerCoutComposite` dans `Voyageur` :

```
coût composite = poids_PRIX × prix + poids_TEMPS × temps + poids_CO2 × co2
```

La méthode `getTrajetsTriesParPreferences` dans `Plateforme` utilise ce coût composite comme critère de tri, en remplaçant le simple `getValeur(critere)` de la V1 par `voyageur.calculerCoutComposite(trajet.getCout())`. Le tri par sélection reste inchangé.

**Historique** : `Voyage` encapsule une liste de `Trajet` et expose `getCoutTotal(TypeCout)` qui somme les coûts de tous les trajets. `Voyageur` maintient une `List<Voyage>` en mémoire et expose `getTotalHistorique(TypeCout)` pour calculer le cumul sur tous les voyages : utile pour montrer à l'utilisateur l'évolution de ses habitudes de voyage.

**Sérialisation binaire** : `HistoriqueManager` utilise `ObjectOutputStream` et `ObjectInputStream` avec try-with-resources pour garantir la fermeture des flux. Si le fichier n'existe pas encore, `charger()` retourne une liste vide plutôt que de lancer une exception. La méthode `ajouterEtSauvegarder` charge l'historique existant, ajoute le nouveau voyage, et réécrit le tout.

---

### 3.4  Analyse des tests

Les tests de la Version 3 ont été ajoutés dans `PlateformeTest`.

**Tests des préférences multi-critères :**
- `getTrajetsTriesParPreferences_retourneTousLesTrajets` : vérification que tous les trajets sont retournés
- `getTrajetsTriesParPreferences_triCroissant` : vérification que le tri est bien croissant sur le coût composite

**Tests de l'historique :**
- `voyage_coutTotalCorrect` : vérification que `getCoutTotal` somme correctement les coûts
- `voyageur_ajoutVoyageDansHistorique` : vérification qu'un voyage est bien ajouté à l'historique
- `voyageur_totalHistoriqueCumule` : vérification que `getTotalHistorique` cumule correctement sur plusieurs voyages

**Tests de la sérialisation :**
- `historiqueManager_sauvegardeEtCharge` : vérification que les données sont correctement écrites et relues depuis le fichier binaire
- `historiqueManager_fichierInexistant_retourneListeVide` : vérification du comportement au premier lancement